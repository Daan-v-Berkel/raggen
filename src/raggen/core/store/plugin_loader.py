...
def load_object(import_path: str) -> Any:
    ...
    if not isinstance(import_path, str) or ":" not in import_path:
        raise BackendLoadError(f"Import path must be a string 'module:Attr', got: {import_path!r}")
    module_path, attr = import_path.split(":", 1)
    if not module_path or not attr:
        raise BackendLoadError(f"Invalid import path '{import_path}'. Expected format 'module:ClassName'.")
    try:
        module = importlib.import_module(module_path)
    except Exception as exc:
        raise BackendLoadError(f"Failed to import module '{module_path}': {exc}") from exc
    try:
        obj = getattr(module, attr)
    except AttributeError as exc:
        raise BackendLoadError(f"Module '{module_path}' has no attribute '{attr}'") from exc
    return obj
...
def load_vector_backend(import_path: str) -> VectorBackend:
    ...
    if inspect.isclass(obj):
        try:
            ...
            raise BackendLoadError(f"Failed to instantiate backend class '{import_path}': {exc}") from exc
    else:
